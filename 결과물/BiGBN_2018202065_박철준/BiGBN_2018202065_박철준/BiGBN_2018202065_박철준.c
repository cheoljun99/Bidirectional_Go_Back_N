...
#define TIMEOUT 1000 //TIME OUT을 10으로 함
    ...
    if (getchecksum(packet) == packet.checksum) 
    {
        ...
        */
        ...
        if (expected_seqnum_A == seqnum)
        {
            ...
            printf("A_input :got ACK and make ACK 즉 B사이드에 보낼 ACK (누적 ack 혹은 실시간 으로 갱신한 ack = %d)\n", A_my_acknum);
    ...
    if (getchecksum(packet) == packet.checksum) 
    {
        ...
        */
        ...
        if (expected_seqnum_B == seqnum) 
        {
            ...
            printf("B_input :got ACK and make ACK 즉 A 사이드에 보낼 ACK (누적 ack 혹은 실시간 으로 갱신한 ack = %d)\n", B_my_acknum);
